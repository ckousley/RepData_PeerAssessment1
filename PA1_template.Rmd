...
```{r}
setwd("~/Coursera/Reproducible Research/Course Project 1/RepData_PeerAssessment")
...
```{r histogram_complete, fig.align='center'}
hist(stepsbyday$steps, main="Total Steps per Day\n(excludes missing values)", xlab="Steps per Day", col="slategray3")
```

Excluding missing values, the individual represented in this dataset averaged `r format(meansteps, scientific=NA, big.mark=",")` **mean** total steps per day and `r format(mediansteps, scientific=NA, big.mark=",")` **median** steps per day.
...
```{r time_series, fig.align='center'}
...
Averaged across all the days, the **`r maxinterval`** time interval contains the maximum number of steps taken.
...
The dataset includes a total of `r missing` rows with missing values (NA).  Missing values are populated using the average steps per time interval across all days with data available.  The imputed dataset reflects the original activity dataset but with missing values filled in.
...
```{r histogram_imputed, fig.align='center'}
hist(stepsbydayimputed$steps, main="Total Steps per Day\n(missing values are imputed)", xlab="Steps per Day", col="slategray3") 
```

With missing values filled in, the **mean** total steps per day remains unchanged at `r format(meanstepsimputed, digits=2, big.mark=",")`.  The **median** steps per day increases slightly from `r format(mediansteps, scientific=NA, big.mark=",")` to `r format(medianstepsimputed, scientific=NA, big.mark=",")`.
...
```{r weekend_weeday, fig.align='center'}
library(lattice)
